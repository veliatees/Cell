...
def main() -> None:
    ...
    state = run_cell(
        ...
        rng=EngineRng(definition.stochastic_policy.seed),
    )
    ...
    integrated_network = build_integrated_hepatocyte_network(HormoneState())
    ...
    kinetic_transfer_audit = build_kinetic_transfer_audit(integrated_network)
    validate_kinetic_transfer_audit(kinetic_transfer_audit)
    reaction_evidence_atlas = build_reaction_evidence_atlas(
        integrated_network,
        kinetic_transfer_audit,
    )
    constraint_shell = metabolic_constraint_shell_snapshot()
    validate_metabolic_constraint_shell(constraint_shell)
    metabolic_cycle_program = hepatocyte_metabolic_cycle_program_snapshot()
    validate_hepatocyte_metabolic_cycle_program(metabolic_cycle_program)
    cytosol_transport = cytosol_transport_snapshot()
    validate_cytosol_transport_snapshot(cytosol_transport)
    completion_matrix = build_hepatocyte_completion_matrix()
    validate_hepatocyte_completion_matrix(completion_matrix)
    software_completion = completion_matrix["software_completion_boundary"]
        ...
        snapshot_to_json(
            ...
            state_extras={
                ...
                "model_authority": {
                    ...
                    "policy": "quantitative_state is static whole-liver context only and cannot initialize one cell, convert counts or run dynamics; organelle_placement is a mixed-species seeded collision/renderer proxy and has no healthy-PHH count, morphology, mechanics or quantitative-contact authority; whole_cell_runtime_authority requires an explicit schematic or exploratory purpose and rejects quantitative, predictive and authoritative state-coupling use of legacy relative-pool dynamics; the capability atlas defines scope but activates no behavior; the completion matrix reports only scoped engineering status and never a whole-cell realism percentage; the software-completion boundary means only that every currently declared non-closed scope has an exact external evidence, external action or inapplicability disposition, never that the hepatocyte or digital twin is scientifically complete; the quantity harvest preserves raw species, model, assay and denominator and cannot activate parameters without explicit review; the PHH injury operator requires exact protocol context, never interprets missing evidence as no effect, and cannot mutate state; donor-resolved injury intake additionally requires a versioned raw schema, donor/study-disjoint splits, manual review, frozen checksums and exact assay projection before descriptive heldout residuals; legacy injury fixtures reject quantitative, predictive and authoritative-coupling use; cellular memory requires a donor-resolved write, verified-removal persistence, first-response and rechallenge-response trajectory plus a frozen held-out-validated law and never treats an event log as memory; cytosol_transport keeps passive aqueous projection separate from deterministic dimensionless cargo rendering, applies outer-membrane cut-cell geometric conservation and subgrid organelle interception only as numerical boundary treatments, forbids a single global viscosity/crowding multiplier and gates fluid coupling per reaction; cytoplasm_dynamics retains cross-context motion observations but exports zero healthy-PHH motion parameters, while browser staging uses only independent renderer-world and wall-clock values; the normalized browser biochemical fixture is isolated to software tests and cannot execute in production under any snapshot state; a missing Python snapshot produces neutral anatomy and explicit unavailability rather than local biochemical dynamics; receptor_signaling_trajectory_intake requires donor-matched density, binding, occupancy, trafficking, response and frozen heldout stages but cannot activate a receptor or signal; active_protein_localization_intake requires donor-matched total, localized, active, denominator and functional measurements but cannot infer active copies or scale rates; metabolic_constraint_shell pins the generic Human-GEM artifact but cannot execute until PHH context, boundaries, objective and validation are frozen; compartmental_energy_redox owns energy/redox pool identity but activates no values or rates; energy_redox_validation quarantines every legacy ATP/redox/OXPHOS parameter until compartment, identifiability and held-out gates pass; external_validation_program owns context-of-use, claim, reviewer-role and independence semantics while assigning no biological-accuracy percentage or predictive claim; spatial_world wins on runtime geometry; reaction_authority prevents topology citations or unsupported rates from entering quantitative validation; kinetic_transfer additionally requires exact stoichiometry, compartment, symbolic equation, per-cell units, context and validation before a published parameter can enter the active network; exact_glucose_homeostasis owns canonical glucose-pool identity while glucose_open_system keeps sinusoid and PHH assay contexts non-interchangeable; glucose_calibration_validation permits descriptive residuals but blocks fitting and prediction until observability and held-out gates pass; geometry may change spatial_state but cannot alter biochemistry until a source-backed interaction law passes validation.",
                },
                "whole_cell_runtime_authority": whole_cell_runtime_authority_snapshot(),
                "legacy_calibration_authority": legacy_calibration_authority_snapshot(),
                ...
                "energy_redox_trajectory_intake": energy_redox_trajectory_intake_snapshot(),
                "completion_evidence_bundle_intake": completion_evidence_bundle_intake_snapshot(),
                "receptor_signaling_trajectory_intake": receptor_signaling_trajectory_snapshot(),
                "active_protein_localization_intake": active_protein_localization_snapshot(),
                "phh_3d_mesh_boundary_intake": phh_3d_mesh_boundary_intake_snapshot(),
                "intracellular_mobility_intake": intracellular_mobility_intake_snapshot(),
                "reaction_transport_coupling_intake": reaction_transport_coupling_intake_snapshot(),
                "external_validation_program": external_validation_snapshot(),
                "hepatocyte_capability_atlas": hepatocyte_capability_atlas_snapshot(),
                "cellular_memory_contract": cellular_memory_contract_snapshot(),
                "reaction_evidence_atlas": reaction_evidence_atlas,
                "reaction_evidence_intake": reaction_evidence_intake_snapshot(),
                "cytosol_transport": cytosol_transport,
                "metabolic_constraint_shell": constraint_shell,
                "hepatocyte_metabolic_cycle_program": metabolic_cycle_program,
                "hepatocyte_completion_matrix": completion_matrix,
                "software_completion_boundary": software_completion,
                "phh_evidence_readiness": evidence_readiness,
                ...
                "hepatocyte_quantity_harvest": hepatocyte_quantity_harvest_snapshot(),
                "phh_injury_validation": phh_injury_validation_snapshot(),
                ...
                "schematic_visual_state": schematic_visual_state_snapshot(
                    tuple(sorted(definition.pool_ids)),
                    runtime_purpose=runtime_purpose,
                    executed_step_count=args.steps,
                ...
                "kinetic_transfer": kinetic_transfer_audit.to_dict(),
                "experiment": {
                    "id": experiment.id,
