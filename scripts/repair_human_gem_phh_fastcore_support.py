...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Compute exact source-limited supports for adaptive PHH FASTCORE "
            "blockers and validate their union with strict FASTCC."
        )
    )
    parser.add_argument("--out", type=Path, default=DEFAULT_AUDIT_PATH)
    args = parser.parse_args()

    report = build_pinned_human_gem_phh_fastcore_support_repair()
