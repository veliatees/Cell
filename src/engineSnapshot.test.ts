...
describe("engine snapshot client", () => {
  ...
  it("accepts the generated checksum-verified human-liver atlas snapshot", async () => {
    ...
    if (result.status === "loaded") {
      ...
      expect(result.summary.cytosolTransport?.version).toBe("cytosol_transport_rheology_contract_v10");
      ...
      expect(result.summary.cytosolTransport?.solver_layers.renderer_dimensionless_projection_grid.mesh_self_intersection_detection).toBe(false);
      ...
      expect(result.summary.phh3dMeshBoundaryIntake?.gates.self_intersection_audit_implemented_in_repository).toBe(false);
