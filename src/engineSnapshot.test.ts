...
describe("engine snapshot client", () => {
  ...
  it("accepts the generated checksum-verified human-liver atlas snapshot", async () => {
    ...
    if (result.status === "loaded") {
      ...
      expect(result.summary.cytosolTransport?.summary.analytic_obstacle_shape_count).toBe(4);
      expect(result.summary.cytosolTransport?.summary.renderer_geometry_boundary_class_count).toBe(10);
      expect(result.summary.cytosolTransport?.summary.generic_watertight_mesh_boundary_kernel_count).toBe(1);
      expect(result.summary.cytosolTransport?.summary.repository_mesh_self_intersection_audit_count).toBe(1);
      expect(result.summary.cytosolTransport?.summary.non_star_shaped_closed_mesh_domain_kernel_count).toBe(1);
      ...
      expect(result.summary.phh3dMeshBoundaryIntake?.gates.self_intersection_audit_implemented_in_repository).toBe(true);
      expect(result.summary.intracellularMobilityIntake?.summary.target_species_count).toBe(43);
      expect(result.summary.intracellularMobilityIntake?.summary.required_stage_slot_count).toBe(387);
      expect(result.summary.intracellularMobilityIntake?.summary.apparent_diffusivity_authorized_species_count).toBe(0);
      expect(result.summary.intracellularMobilityIntake?.summary.crowding_law_authorized_species_count).toBe(0);
      expect(result.summary.reactionTransportCouplingIntake?.summary.target_reaction_count).toBe(36);
      expect(result.summary.reactionTransportCouplingIntake?.summary.required_stage_slot_count).toBe(288);
      expect(result.summary.reactionTransportCouplingIntake?.summary.local_concentration_coupled_reaction_count).toBe(0);
      expect(result.summary.reactionTransportCouplingIntake?.summary.global_fluid_multiplier_count).toBe(0);
      ...
      expect(result.summary.metabolicConstraintShell?.version).toBe("metabolic_constraint_shell_v15");
      expect(
        result.summary.metabolicConstraintShell?.generic_dynamic_fba_numerics
          .summary.registered_dynamic_fba_update_law_count
      ).toBe(1);
      expect(
        result.summary.metabolicConstraintShell?.generic_dynamic_fba_numerics
          .summary.analytic_fixture_pass_count
      ).toBe(6);
      expect(
        result.summary.metabolicConstraintShell?.generic_dynamic_fba_numerics
      ...
      expect(result.summary.hepatocyteMetabolicCycleProgram?.version).toBe(
        "hepatocyte_metabolic_cycle_program_v1"
      );
      expect(
        result.summary.hepatocyteMetabolicCycleProgram?.summary.cycle_count
      ).toBe(4);
      expect(
        result.summary.hepatocyteMetabolicCycleProgram?.summary
      ...
      expect(
        result.summary.hepatocyteMetabolicCycleProgram
      ...
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.model_loader_verified_against_pinned_artifact).toBe(true);
      ...
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.generic_flux_consistency_audit.lp7_solve_count).toBe(6);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.generic_flux_consistency_audit.lp3_solve_count).toBe(247);
      ...
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.seven_donor_proteome_gpr_audit.flux_consistent_core_candidate_count).toBe(4555);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.seven_donor_fastcore_trial.source_fastcore_selected_reaction_count).toBe(7320);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.seven_donor_fastcore_trial.source_fastcore_output_blocked_reaction_count).toBe(408);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.seven_donor_fastcore_trial.closure_selected_reaction_count).toBe(11639);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.seven_donor_fastcore_trial.context_model_accepted).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.seven_donor_gpr_stability_audit.six_donor_support_reaction_count).toBe(150);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.seven_donor_gpr_stability_audit.largest_leave_one_out_core_expansion_count).toBe(62);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_scaling_comparison.adaptive_selected_reaction_count).toBe(7415);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_scaling_comparison.adaptive_output_blocked_reaction_count).toBe(17);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_scaling_comparison.context_model_accepted).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_blocker_diagnostics.full_network_active_blocker_count).toBe(17);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_blocker_diagnostics.candidate_blocked_reaction_count).toBe(17);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_support_repair.added_reaction_union_count).toBe(65);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_support_repair.repaired_candidate_reaction_count).toBe(7480);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_support_repair.strict_fastcc_blocked_reaction_count).toBe(0);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_support_repair.reaction_activity_in_phh_established).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_shared_support.minimum_shared_added_reaction_count).toBe(59);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_shared_support.removed_from_per_target_union_count).toBe(6);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_shared_support.repaired_candidate_reaction_count).toBe(7474);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_shared_support.strict_fastcc_blocked_reaction_count).toBe(0);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_shared_support.reaction_activity_in_phh_established).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_support_optimality.minimum_support_set_count).toBe(2);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_support_optimality.reactions_present_in_every_minimum_support_count).toBe(58);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_support_optimality.optional_reaction_ids_in_input_order).toEqual(["MAR02308", "MAR10035"]);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_support_optimality.terminal_infeasibility_proven).toBe(true);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_optimality.global_candidate_reaction_count).toBe(4226);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_optimality.lower_bound_target_ids_in_input_order).toEqual(["MAR00468", "MAR00612"]);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_optimality.lower_bound_exact_minimum_added_reaction_count).toBe(59);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_optimality.upper_bound_feasible_added_reaction_count).toBe(59);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_optimality.bounds_match).toBe(true);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_optimality.global_minimum_cardinality_proven).toBe(true);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_optimality.global_minimum_identity_sets_enumerated).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_optimality.global_minimum_support_set_unique).toBeNull();
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_optimality.reaction_activity_in_phh_established).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.global_candidate_reaction_count).toBe(4226);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.global_minimum_cardinality).toBe(59);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.known_distinct_global_minimum_support_set_count_lower_bound).toBe(3);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.presolve_infeasibility_disagreed).toBe(true);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.solver_attempt_count).toBe(2);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.accepted_solve_used_presolve).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.counterexample_only_reaction_ids_in_input_order).toEqual(["MAR00494"]);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.outside_scoped_pool_reaction_ids_in_input_order).toEqual(["MAR00494"]);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.all_target_lp_certificate_count).toBe(17);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.strict_fastcc_blocked_reaction_count).toBe(0);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.global_minimum_identity_enumeration_complete).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.global_minimum_support_set_unique).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_counterexample.reaction_activity_in_phh_established).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_fixed_core_completion_enumeration.fixed_common_reaction_count).toBe(58);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_fixed_core_completion_enumeration.conditioned_retained_reaction_count).toBe(7473);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_fixed_core_completion_enumeration.remaining_candidate_reaction_count).toBe(4168);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_fixed_core_completion_enumeration.known_singleton_completion_ids_in_model_order).toEqual(["MAR00494", "MAR02308", "MAR10035"]);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_fixed_core_completion_enumeration.terminal_infeasibility_confirmed_without_presolve).toBe(true);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_fixed_core_completion_enumeration.exact_singleton_completion_count_given_fixed_core).toBe(3);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_fixed_core_completion_enumeration.fixed_core_singleton_completion_enumeration_complete).toBe(true);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_fixed_core_completion_enumeration.global_minimum_identity_enumeration_complete).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_fixed_core_completion_enumeration.multi_replacement_global_optima_excluded).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_fixed_core_completion_enumeration.reaction_activity_in_phh_established).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_identity_completeness.global_candidate_reaction_count).toBe(4226);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_identity_completeness.global_minimum_added_reaction_count).toBe(59);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_identity_completeness.global_minimum_support_set_count).toBe(3);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_identity_completeness.global_minimum_support_identity_enumeration_complete).toBe(true);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_identity_completeness.global_universal_minimum_support_reaction_count).toBe(58);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_identity_completeness.global_optional_minimum_support_reaction_ids_in_model_order).toEqual(["MAR00494", "MAR02308", "MAR10035"]);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_identity_completeness.multi_replacement_global_optima_excluded).toBe(true);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_identity_completeness.additional_global_minimum_identity_search_required).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_identity_completeness.terminal_infeasibility_confirmed_without_presolve).toBe(true);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_identity_completeness.structural_essentiality_at_larger_support_sizes_established).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.fastcore_global_support_identity_completeness.reaction_activity_in_phh_established).toBe(false);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.reaction_evidence_manifest.manifest_reaction_count).toBe(4895);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.reaction_evidence_manifest.adaptive_noncore_without_gpr_count).toBe(2177);
      expect(result.summary.metabolicConstraintShell?.candidate_reconstruction.reaction_evidence_manifest.automatic_bound_change_allowed).toBe(false);
      ...
      expect(result.summary.metabolicConstraintShell?.context_extraction_kernel.algorithm).toBe("FASTCORE");
      expect(result.summary.metabolicConstraintShell?.context_extraction_kernel.synthetic_fixture_pass_count).toBe(1);
      expect(result.summary.metabolicConstraintShell?.context_extraction_kernel.human_gem_context_extraction_executed).toBe(false);
      expect(result.summary.metabolicConstraintShell?.context_extraction_kernel.official_adaptive_lp10_supported).toBe(true);
      expect(result.summary.metabolicConstraintShell?.context_extraction_kernel.official_adaptive_lp10_core_multiplier).toBe(10);
      ...
      expect(result.summary.hepatocyteCompletionMatrix?.summary.entry_count).toBe(60);
      expect(result.summary.hepatocyteCompletionMatrix?.summary.closed_count).toBe(35);
      ...
      expect(result.summary.evidenceReadiness?.summary.registry_contract_count).toBe(16);
      expect(result.summary.evidenceReadiness?.summary.contract_identity_verified_count).toBe(16);
      expect(result.summary.evidenceReadiness?.summary.validator_surface_count).toBe(16);
      expect(result.summary.evidenceReadiness?.summary.target_gap_count).toBe(23);
      expect(result.summary.evidenceReadiness?.summary.rejected_intake_count).toBe(0);
      expect(result.summary.evidenceReadiness?.summary.quantitatively_authorized_item_count).toBe(0);
      expect(result.summary.evidenceReadiness?.summary.automatic_parameter_activation_count).toBe(0);
      expect(result.summary.evidenceReadiness?.summary.automatic_state_coupling_count).toBe(0);
      expect(result.summary.completionEvidenceBundleIntake?.summary.required_table_count).toBe(3);
      expect(result.summary.completionEvidenceBundleIntake?.summary.required_capability_slot_count).toBe(44);
      expect(result.summary.completionEvidenceBundleIntake?.summary.quantitatively_authorized_item_count).toBe(0);
      expect(result.summary.softwareCompletionBoundary?.summary.evidence_gated_scope_count).toBe(23);
      expect(result.summary.softwareCompletionBoundary?.summary.registered_evidence_gated_scope_count).toBe(23);
      expect(result.summary.softwareCompletionBoundary?.summary.responsible_code_only_scope_count).toBe(0);
      expect(result.summary.softwareCompletionBoundary?.scientific_model_complete).toBe(false);
      expect(result.summary.softwareCompletionBoundary?.biological_validation_complete).toBe(false);
      expect(result.summary.softwareCompletionBoundary?.digital_twin_predictive_authority).toBe(false);
      ...
      expect(result.summary.phhInjuryValidation?.validation_data_contract.conditional_columns).toHaveLength(10);
      ...
      expect(result.summary.phhInjuryValidation?.trajectory_intake.expected_header_count).toBe(29);
      expect(result.summary.phhInjuryValidation?.trajectory_intake.split_leakage_guard_enabled).toBe(true);
      expect(result.summary.phhInjuryValidation?.trajectory_intake.independent_heldout_study_guard_enabled).toBe(true);
      expect(result.summary.phhInjuryValidation?.trajectory_intake.measurement_operator.exact_match_dimensions).toHaveLength(20);
      expect(result.summary.phhInjuryValidation?.trajectory_intake.measurement_operator.numeric_projection_ready).toBe(false);
      expect(result.summary.phhInjuryValidation?.trajectory_intake.frozen_evaluation.required_submission_fields).toHaveLength(12);
      expect(result.summary.phhInjuryValidation?.trajectory_intake.frozen_evaluation.evaluation_ready).toBe(false);
      expect(result.summary.phhInjuryValidation?.summary.trajectory_intake_validator_count).toBe(1);
      expect(result.summary.phhInjuryValidation?.summary.exact_assay_projection_operator_count).toBe(1);
      expect(result.summary.phhInjuryValidation?.summary.numeric_measurement_projection_count).toBe(0);
      expect(result.summary.phhInjuryValidation?.summary.independent_heldout_result_count).toBe(0);
