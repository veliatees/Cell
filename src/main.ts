...
function renderEvidenceBoundary(summary: EngineSnapshotSummary | null): string {
  ...
  const mechanicsAndConstraintRow = (cytosolTransport && metabolicConstraint
    ? `<div class="phh-profile"><div class="phh-profile__head"><b>Mechanics + constraint execution plane v2</b><span>verified model loading · source-defined context numerics · biological execution blocked</span></div><div class="phh-profile__grid"><span>Mechanics evidence columns <b>${cytosolTransport.phh_mechanics_calibration_intake.expected_header_count}</b></span><span>Mechanics quantities targeted <b>${cytosolTransport.summary.phh_mechanics_target_quantity_count}</b></span><span>Delivered / spatial-FSI-ready trajectories <b>${cytosolTransport.summary.delivered_phh_mechanics_trajectory_count} / ${cytosolTransport.summary.spatial_fsi_ready_phh_mechanics_trajectory_count}</b></span><span>Authorized mechanics parameters <b>${cytosolTransport.summary.quantitatively_authorized_phh_mechanics_parameter_count}</b></span><span>Human-GEM sparse terms loaded <b>${metabolicConstraint.candidate_reconstruction.sparse_fbc_loader_audit.stoichiometric_nonzero_count.toLocaleString()}</b></span><span>Gene-associated reactions <b>${metabolicConstraint.candidate_reconstruction.sparse_fbc_loader_audit.gene_associated_reaction_count.toLocaleString()}</b></span><span>LP backend <b>${metabolicConstraint.generic_constraint_numerics.backend_version} · ${metabolicConstraint.generic_constraint_numerics.method}</b></span><span>FBA/FVA fixtures passed <b>${metabolicConstraint.generic_constraint_numerics.analytic_fixture_pass_count}</b></span><span>FASTCORE fixtures passed <b>${metabolicConstraint.context_extraction_kernel.synthetic_fixture_pass_count}</b></span><span>Healthy-PHH context extraction <b>${metabolicConstraint.context_extraction_kernel.human_gem_context_extraction_executed ? "executed" : "blocked"}</b></span><span>PHH execution artifacts required <b>${metabolicConstraint.phh_execution_bundle_intake.required_artifact_count}</b></span><span>Delivered PHH execution bundles <b>${metabolicConstraint.phh_execution_bundle_intake.delivered_bundle_count}</b></span><span>Measured exchange bounds <b>${metabolicConstraint.phh_execution_bundle_intake.measured_exchange_bound_count}</b></span><span>Runtime flux coupling <b>${metabolicConstraint.phh_execution_bundle_intake.runtime_flux_coupling_allowed ? "enabled" : "blocked"}</b></span></div></div><div class="evidence-row"><span class="evidence-tag evidence-tag--derived">Execution firewall</span><span>The checksum-pinned generic Human-GEM structure now loads exactly, and the FBA/FVA plus FASTCORE kernels pass synthetic numerical checks. They remain separate from PHH biology until donor-resolved mechanics and a frozen healthy-PHH context bundle with an evidence-backed core set, measured bounds, objective, scale operator and independent flux validation are delivered.</span></div>`
    : "").replace(
