...
function renderEvidenceBoundary(summary: EngineSnapshotSummary | null): string {
  ...
  const metabolicSupportRepairRow = metabolicConstraint
    ? `<div class="phh-profile phh-profile--mechanics-execution">
        <div class="phh-profile__head"><b>PHH structural support audit v4</b><span>global cardinality · scoped identity enumeration · strict FASTCC</span></div>
        <div class="phh-profile__grid">
          ...
          <span>Global identity sets enumerated <b>${metabolicConstraint.candidate_reconstruction.fastcore_global_support_optimality.global_minimum_identity_sets_enumerated ? "yes" : "no"}</b></span>
      ...
      <div class="evidence-row"><span class="evidence-tag evidence-tag--derived">Structural, not biological</span><span>An exact two-target lower bound over all 4,226 omitted reactions matches the feasible 17-target upper bound at 59, proving global minimum cardinality. The two identity sets, 58 common reactions and MAR02308/MAR10035 interchangeability are complete only inside the 65-reaction pool; global identity alternatives remain unenumerated. No active PHH enzyme, exchange bound, biological objective, FBA output or runtime rate is established.</span></div>`
    : "";
