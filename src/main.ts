...
function renderEvidenceBoundary(summary: EngineSnapshotSummary | null): string {
  ...
  const metabolicSupportRepairRow = metabolicConstraint
    ? `<div class="phh-profile phh-profile--mechanics-execution"><div class="phh-profile__head"><b>PHH structural support repair v1</b><span>17 blockers · exact MILP · strict FASTCC</span></div><div class="phh-profile__grid"><span>Raw adaptive selected / blocked <b>${metabolicConstraint.candidate_reconstruction.fastcore_scaling_comparison.adaptive_selected_reaction_count.toLocaleString()} / ${metabolicConstraint.candidate_reconstruction.fastcore_scaling_comparison.adaptive_output_blocked_reaction_count}</b></span><span>Generic-active blockers diagnosed <b>${metabolicConstraint.candidate_reconstruction.fastcore_blocker_diagnostics.full_network_active_blocker_count}/${metabolicConstraint.candidate_reconstruction.fastcore_blocker_diagnostics.diagnosed_blocker_count}</b></span><span>Directional minimum-support MILPs <b>${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.direction_milp_solve_count}</b></span><span>Per-target additions min / max <b>${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.minimum_per_target_added_reaction_count} / ${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.maximum_per_target_added_reaction_count}</b></span><span>Unique support union <b>${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.added_reaction_union_count}</b></span><span>Repaired candidate reactions <b>${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.repaired_candidate_reaction_count.toLocaleString()}</b></span><span>Strict FASTCC consistent / blocked <b>${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.repaired_candidate_reaction_count.toLocaleString()} / ${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.strict_fastcc_blocked_reaction_count}</b></span><span>Added no-GPR / zero-donor GPR <b>${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.added_reaction_without_gpr_count} / ${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.added_reaction_zero_donor_gpr_count}</b></span><span>PHH activity established <b>${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.reaction_activity_in_phh_established ? "yes" : "no"}</b></span><span>Context accepted / PHH FBA <b>${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.context_model_accepted ? "yes" : "no"} / ${metabolicConstraint.candidate_reconstruction.fastcore_support_repair.fba_execution_allowed ? "enabled" : "blocked"}</b></span></div></div><div class="evidence-row"><span class="evidence-tag evidence-tag--derived">Structural, not biological</span><span>The 65-reaction union uses no external reaction database and changes no generic Human-GEM bound. It makes all 7,480 candidate reactions flux-consistent, but all 65 additions still require PHH activity evidence; numerical repair does not authorize a healthy-hepatocyte objective, exchange state, FBA result or runtime rate.</span></div>`
    : "";
