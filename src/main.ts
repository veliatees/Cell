...
function renderEvidenceBoundary(summary: EngineSnapshotSummary | null): string {
  ...
  const mechanicsAndConstraintRow = (cytosolTransport && metabolicConstraint
    ? `<div class="phh-profile"><div class="phh-profile__head"><b>Mechanics + constraint execution plane v3</b><span>generic reconstruction audited · healthy-PHH execution blocked</span></div><div class="phh-profile__grid"><span>Mechanics evidence columns <b>${cytosolTransport.phh_mechanics_calibration_intake.expected_header_count}</b></span><span>Mechanics quantities targeted <b>${cytosolTransport.summary.phh_mechanics_target_quantity_count}</b></span><span>Delivered / spatial-FSI-ready trajectories <b>${cytosolTransport.summary.delivered_phh_mechanics_trajectory_count} / ${cytosolTransport.summary.spatial_fsi_ready_phh_mechanics_trajectory_count}</b></span><span>Authorized mechanics parameters <b>${cytosolTransport.summary.quantitatively_authorized_phh_mechanics_parameter_count}</b></span><span>Human-GEM sparse terms loaded <b>${metabolicConstraint.candidate_reconstruction.sparse_fbc_loader_audit.stoichiometric_nonzero_count.toLocaleString()}</b></span><span>Gene-associated reactions <b>${metabolicConstraint.candidate_reconstruction.sparse_fbc_loader_audit.gene_associated_reaction_count.toLocaleString()}</b></span><span>Generic flux-consistent reactions <b>${metabolicConstraint.candidate_reconstruction.generic_flux_consistency_audit.consistent_reaction_count.toLocaleString()}</b></span><span>Generic blocked reactions <b>${metabolicConstraint.candidate_reconstruction.generic_flux_consistency_audit.blocked_reaction_count.toLocaleString()}</b></span><span>FASTCC witness residual <b>${metabolicConstraint.candidate_reconstruction.generic_flux_consistency_audit.maximum_mass_balance_residual.toExponential(2)}</b></span><span>Generic native objective <b>${metabolicConstraint.candidate_reconstruction.generic_native_objective_audit.status}</b></span><span>Generic objective value <b>${metabolicConstraint.candidate_reconstruction.generic_native_objective_audit.objective_value.toFixed(6)}</b></span><span>Generic active reactions <b>${metabolicConstraint.candidate_reconstruction.generic_native_objective_audit.active_reaction_count_at_1e_minus_9.toLocaleString()}</b></span><span>LP backend <b>${metabolicConstraint.generic_constraint_numerics.backend_version} · ${metabolicConstraint.generic_constraint_numerics.method}</b></span><span>FBA/FVA fixtures passed <b>${metabolicConstraint.generic_constraint_numerics.analytic_fixture_pass_count}</b></span><span>FASTCORE fixtures passed <b>${metabolicConstraint.context_extraction_kernel.synthetic_fixture_pass_count}</b></span><span>Healthy-PHH context extraction <b>${metabolicConstraint.context_extraction_kernel.human_gem_context_extraction_executed ? "executed" : "blocked"}</b></span><span>PHH execution artifacts required <b>${metabolicConstraint.phh_execution_bundle_intake.required_artifact_count}</b></span><span>Delivered PHH execution bundles <b>${metabolicConstraint.phh_execution_bundle_intake.delivered_bundle_count}</b></span><span>Measured exchange bounds <b>${metabolicConstraint.phh_execution_bundle_intake.measured_exchange_bound_count}</b></span><span>Runtime flux coupling <b>${metabolicConstraint.phh_execution_bundle_intake.runtime_flux_coupling_allowed ? "enabled" : "blocked"}</b></span></div></div><div class="evidence-row"><span class="evidence-tag evidence-tag--derived">Execution firewall</span><span>The exact generic Human-GEM network is now checksum-loaded, completely FASTCC-classified and solved under its own generic biomass objective. These are reconstruction and solver audits, not healthy-hepatocyte fluxes. PHH execution remains blocked until a frozen donor-resolved context bundle supplies an evidence-backed core set, measured exchange bounds, a PHH objective, scale operator and independent validation.</span></div>`
    : "").replace(
