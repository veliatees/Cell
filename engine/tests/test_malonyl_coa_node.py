...
class MalonylCoANodeTests(unittest.TestCase):
    def test_malonyl_coa_inhibits_cpt1_beta_oxidation(self):
        ...
        def flux(malonyl: float) -> float:
            out = run_malonyl_node(FASTED, 8.0, EngineRng(72),
                                   malonyl_coa=malonyl, fatty_acids=20000.0, params=off)
            return out["mito_acetyl_CoA"]

        none, at_ki, high = flux(0.0), flux(300.0), flux(5000.0)
    ...
    def test_fed_does_lipogenesis_fasted_does_not(self):
        ...
        fed = run_malonyl_node(FED, 80.0, EngineRng(71), fatty_acids=20000.0)
        fasted = run_malonyl_node(FASTED, 80.0, EngineRng(71), fatty_acids=20000.0)
        self.assertGreater(fed["palmitate"], 1000.0)
        self.assertLess(fasted["palmitate"], 100.0)
    ...
    def test_fed_suppresses_beta_oxidation(self):
        ...
        lowers beta-oxidation flux relative to fasted."""
        fed = run_malonyl_node(FED, 80.0, EngineRng(73), fatty_acids=20000.0)
        fasted = run_malonyl_node(FASTED, 80.0, EngineRng(73), fatty_acids=20000.0)
        self.assertGreater(fasted["mito_acetyl_CoA"], 1.2 * fed["mito_acetyl_CoA"])
