...
class DeathBasicsTests(unittest.TestCase):
    def test_healthy_cell_survives(self):
        ...
        final = run_death(healthy, 200.0, purpose="exploratory_execution")
    ...
    def test_stress_with_atp_intact_is_apoptosis(self):
        ...
        final = run_death(stressed, 300.0, purpose="exploratory_execution")
    ...
    def test_atp_collapse_is_necrosis(self):
        ...
        final = run_death(collapsed, 300.0, purpose="exploratory_execution")
    ...
    def test_commitment_is_irreversible(self):
        ...
        committed = run_death(stressed, 300.0, purpose="exploratory_execution")
        self.assertTrue(committed.committed)
        recovered = step_death(
            committed,
            StressSignals(),
            100.0,
...
class IntegratedDeathTests(unittest.TestCase):
    def test_paracetamol_overdose_is_necrosis(self):
        ...
        alive = run_death(
            signals_from_detox(therapeutic, 10000.0),
            300.0,
            purpose="exploratory_execution",
        )
        dead = run_death(
            signals_from_detox(overdose, 10000.0),
            300.0,
            purpose="exploratory_execution",
        )
    ...
    def test_high_viral_load_is_apoptosis(self):
        ...
        dead = run_death(
            signals_from_infection(infected),
            300.0,
            purpose="exploratory_execution",
        )
        alive = run_death(
            signals_from_infection(uninfected),
            300.0,
            purpose="exploratory_execution",
        )
